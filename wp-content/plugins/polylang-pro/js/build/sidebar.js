    ...
    if (!(0,external_lodash_.isEmpty)(templateId)) {
      ...
      external_this_wp_apiFetch_default()({
        ...
        if (!(0,external_lodash_.isEmpty)(template)) {
          ...
          // always returns back to the template pages list because user's custom template pages doesn't exist anymore after deletion.
          // For template pages provided by the theme as a file, template pages deletion remove user's customisation.
          // So it's consistent to redirect to the template pages list too.

          if (isDefaultLanguage(removedTranslation.lang.slug)) {
            // Simply remove the postId query string parameter from the URL to redirect to the template pages list.
...
class MetaBox extends external_this_wp_element_.Component {
  ...
  render() {
    if (!this.props.isAllowedPostType) {
      ...
      }, (0,external_this_wp_i18n_.__)('Templates are not translatable, only template pages are.', 'polylang-pro')));
    }
