...
class PLL_FSE_Recreate_Language extends PLL_FSE_Abstract_Module implements PLL_Module_Interface {
	...
	public function init() {
		if ( ! method_exists( $this->model, 'set_language_in_mass' ) ) {
			return $this;
		}
	...
	public function reassign_language( $args ) {
		...
		$def_lang = $this->model->get_language( $this->options['default_lang'] );
		...
		if ( ! empty( $set_language_post_ids ) ) {
			// Assign the language.
			$this->model->set_language_in_mass( 'post', $set_language_post_ids, $new_lang );
		}
		...
		if ( ! empty( $create_groups_post_ids ) ) {
			...
			$this->model->set_translation_in_mass( 'post', $create_groups_post_ids );
		}
