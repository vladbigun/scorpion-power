...
class PLL_FSE_Query_Filters extends PLL_FSE_Abstract_Module implements PLL_Module_Interface {
	...
	public function init() {
		/**
		 * Filter the query to allow the use of translated template pages.
	...
	/**
	 * Filters the query to allow the use of translated template pages.
	 *
	 * @since 3.2
	 *
	...
	public function translate_template_query( &$query ) {
		...
		if ( $requested_lang['language']->slug === $def_lang->slug ) {
			// No new template pages to look for when the current language is the default one.
			return;
		}
